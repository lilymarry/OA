...
@interface JXCAddChuKuChooseView (){
    
}

@end

@implementation JXCAddChuKuChooseView
                                             ...
                                             closedHanld:(void (^)())closedHandle
{
    ...
    //初始化
    NSArray* nibView =  [[NSBundle mainBundle] loadNibNamed:@"JXCAddChuKuChooseView" owner:nil options:nil];
    JXCAddChuKuChooseView *view= [nibView lastObject];
...
- (IBAction)confirm:(id)sender {
    ...
    if (self.addChooseViewDelegate!=nil) {
        [self.addChooseViewDelegate confirmWithCangkuIndex:self.cangkuIndex AndDuoweiIndex:self.duoweiIndex AndShuliang:self.tf_ShuLiang.text AndZhongliang:self.tf_zhongliang.text];
    }
}
