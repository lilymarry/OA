...
#import "ShenHeAboutViewController.h"
...
#import "TimePicker.h"
#import "ShenHeAboutCell.h"
@interface ShenHeAboutViewController ()
{
...
@implementation ShenHeAboutViewController
...
- (IBAction)showTheChooseView:(id)sender {
    [self showThebgview];
    //  ShenHeChooseView *view =[[ShenHeChooseView alloc]  instanceChooseViewHidChooseViewWithTag:selectedTag];
    //  self.closingLittleNotification=view;
    //  view.shenHeSelectDelegate=self;
    int y=20;
...
-(void)removeTheView:(UITapGestureRecognizer*)gesture{
    [self hidThebgview];
    //   NSObject<ShenHeNotification> *tmpDele=self.closingLittleNotification;
    //  [tmpDele closingTheLittleMenu:nil];
    //    NSArray *viewArr=[self.view subviews];
    //    NSLog(@"111 %@",viewArr);
    //    for  (UIView *pick  in viewArr  ) {
    //        if ([pick isKindOfClass:[TimePicker class]]) {
    //            [(TimePicker *)  pick removeFromSuperview];
    //        }
...
-(void)selectTheTypeID:(int)tid{
    [self hidThebgview];
    // NSObject<ShenHeNotification> *tmpDele=self.closingLittleNotification;
    //  [tmpDele closingTheLittleMenu:^(){
    selectedTag=tid;
    
    switch (tid) {
        case 1:
            self.navigationItem.title=@"相关审核-财务";
            break;
        case 2:
            self.navigationItem.title=@"相关审核-人事";
            break;
        case 3:
            self.navigationItem.title=@"相关审核-行政";
            break;
        case 4:
            self.navigationItem.title=@"相关审核-业务";
            break;
        case 5:
            self.navigationItem.title=@"相关审核-生产";
            break;
        case 6:
            self.navigationItem.title=@"相关审核-全部";
            break;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ShenHeAboutCell *cell = [tableView dequeueReusableCellWithIdentifier:@"shenheabout" forIndexPath:indexPath];
