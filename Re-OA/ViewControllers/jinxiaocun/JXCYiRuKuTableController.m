#import "JXCYiRuKuTableController.h"
#import "MJRefresh.h"
...
#import "JXCDetailRuKuTableController.h"
#import "UserPermission.h"




@interface JXCYiRuKuTableController (){
    NSMutableArray *dataArr;
    BOOL isPull;
}
@end

@implementation JXCYiRuKuTableController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    self.tabBarController.navigationItem.title=@"已入库";
...
-(void)requestListDataByrnum:(NSString *)rnum{
    ...
    [JinXiaoCunWebAPI requestYiRuKuListWithuid:[UserPermission standartUserInfo].ID
                                       Andrnum:rnum
                                       success:^(NSArray *arr){
        if ( arr.count>0) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    JXCDetailRuKuTableController *controller = [storyboard instantiateViewControllerWithIdentifier:@"jxcyiruku"];
    JXCCommonListModel *model = [dataArr objectAtIndex:indexPath.row];
