#import "JXCWeiChuKuTableController.h"
#import "MJRefresh.h"
...
@interface JXCWeiChuKuTableController ()<CommonNotification>{
    NSMutableArray *dataArr;
    BOOL isPull;
}
@end

@implementation JXCWeiChuKuTableController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    self.tabBarController.navigationItem.title=@"未出库";
...
-(void)requestListDataByrnum:(NSString *)rnum{
    ...
    [JinXiaoCunWebAPI getBangDanListWithRnum:rnum bdtyp:@"1" ywtyp:@"2" success:^(NSArray *arr){
        if ( arr.count>0) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    JXCDoChuKuTableController *controller = [storyboard instantiateViewControllerWithIdentifier:@"jxcDoChuku"];
    JXCCommonListModel *model = [dataArr objectAtIndex:indexPath.row];
