...
#import <BMapKit.h>
@interface ZhizhangShangBaoViewController ()<okButtonDelegate>
{
...
@implementation ZhizhangShangBaoViewController
-(void)getlocation
{
...
- (IBAction)submit:(id)sender
{
    ...
    else if ([_lab_ref.text isEqualToString:@"0.000"] ||_tf_report.text.length==0)
    {
        ...
        [alert show];
            ...
            [ShangBaoWebAPI zhizhangShangbao:[UserPermission standartUserInfo].ID andfpid:dic[@"fpId"] x:lo  y:la khid:@"" khnm:_tf_kehuName.text pbid:cengData[@"pbid"] pbnm:cengData[@"pbnm"] pmyi:pmid1 pmer:pmid2 pmsan:pmid3 pmsi:pmid4 pmwu:pmid5 pmliu:pmid6 pmqi:pmid7 refer:_lab_ref.text report:_tf_report.text success:^(NSArray *arr){
                [AlertHelper hideAllHUDsForView:self.view];
                _bt_shangbao.enabled=YES;
