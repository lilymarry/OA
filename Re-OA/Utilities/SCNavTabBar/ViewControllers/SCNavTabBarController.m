...
- (void)viewInit
{
    ...
    NSLog(@"---%@ %@ %@ %@ ",_nowTime ,_stTime ,_etTime ,_state);
    _navTabBar = [[SCNavTabBar alloc] initWithFrame:CGRectMake(DOT_COORDINATE, DOT_COORDINATE, SCREEN_WIDTH, 80) showArrowButton:_showArrowButton];
    ...
    _navTabBar.nowTime=_nowTime;
    _navTabBar.stTime=_stTime;
    _navTabBar.etTime=_etTime;
    _navTabBar.state=_state;
...
{
    [self disPlayView];
    [self.delegate selectTime1:st andEndTime:et nowTime:nowTime andState:sta];
    
}
-(void)disPlayView
{
    [_mainView setContentOffset:CGPointMake(0,0) animated:YES];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
   ...
   // NSLog(@"WWWW %ld",(long)_currentIndex);
